...
const listFoods = (recipe) => {
  let result = [];
  let names = ['Gruffalo', 'oats', 'brown', 'flour', 'pure', 'chopped', 'baking soda', 'baking powder', 'cinnamon', 'melted butter', 'fresh water'];

  names.forEach(nameValue => {
    recipe.ingredients.forEach(value => {
      if (value.includes(nameValue)) {
        let x = value.indexOf(nameValue);
        let y = value.slice(x);
        result.push(y);
      }
    })
