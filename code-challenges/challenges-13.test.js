...
const longestString = (arr) => {
  return arr.indexOf(arr.reduce((acc, val) =>
...
------------------------------------------------------------------------------------------------ */
const firstLetters = (arr) => arr.map(val => val.slice(0, 1))
...
const allHappy = (arr) => (arr.filter(val => val.includes(':)')).length === arr.length) ? true : false;
