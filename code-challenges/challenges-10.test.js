...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(val1 => val1.filter(val2 => val2 % 5 === 0 && typeof val2 === 'number' ? 1 : 0).map(val3 => val3 = Math.pow(2, val3)))
};
...
let findMaleAndFemale = (data) => {
  return data.filter(val => val.gender === 'male' || val.gender === 'female' ? 1 : 0).reduce((acc, val2) => {
    acc.push(val2.name)
    return acc
  }, []).join(' and ')
};
